...
/*dir - is direction of player's snake that is set by pressed keyboards in STANDARD mode */
...
/*
update_demo_mode, it wokrs if game state equals to DEMO state.
	It creates two snakes with simple AI and lets them play.
	There is a while loop in which game states are updated and
	then are rendered. Demo mode ends if some snake died or
	if [x] is pressed.
@params 
...
/*
Input, it sets global variable dir means direction of snake 
	based on pressed keyboard. This function is called only in Standard
	mode and only snake ONE can be controled by this Input. 
	[W] = UP, [S] = DOWN, [D] = RIGHT, [A] = LEFT.
	[X] Ends the Standard game mode.
*/
void input();

/*
update_standard_mode, it wokrs if game state equals to STANDARD state.
	It creates two snakes one with AI and other controled by fc. Input.
	There is a while loop in which game states are updated and
	then are rendered. Standard mode ends if some snake died or
	if [x] is pressed.
@params 
	Score - to save score of snakes
	Game_Properties - to know game properties
...
/*
update_menu_option, it wokrs if game state equals to MENU state
	or Options state. In menu there are for buttons that can be selected.
	[1] DEMO mode, [2] STANDARD mode, [3] Options and [4] Quit.
	[1] Starts demo mode, [2] starts standard mode, [3] opens options
	in which some parameters of game can be changed. [4] Ends game.
	In options there are five buttons. [1] Speed - sets speed from 1 to 5
	of move in game (one second to 0.2 of second). [2] - sets size of tile
	in pixels of game [16..20..32]. [3] sets color of first snake. [4] sets
	color of second snake. [5] back to menu (back to menu is also represented
	by pressed key X).
	In both Menu and Options selection is represented by pressed keys [W] = up,
...
/*
update_menu_option, it wokrs if game state equals to GameOver state.
	It renders status of snakes actually their score with their color.
	In this state if there is pressed [X] or [F] it returns to the Menu.
@params 
...
/*
loading function starts at first of game and here is just for fun.
	To skip this press [F]
*/
void loading();
	...
	clear_array(BLACK, fb);
			if(kbhit()){
				if(gm_state==Menu){
					...
					int select = menu_options->options_selection(getch(), game_properties.speed, game_properties.size_of_tile, game_properties.color_1, game_properties.color_2, mem_base);
					if (select ==1){
						gm_state = Menu;
					}
				}
