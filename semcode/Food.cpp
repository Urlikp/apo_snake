...
#include "Food.h"
/*
All functions are described in Food.h
*/
...
void Food::fill_array(uint16_t* fb, int LCD_width)
{
    ...
    {
        for (int j = food_x * tile_size; j < food_x * tile_size + tile_size; 
        	j++) 
        {
            fb[LCD_width * i + j] = color;
        }
    }
    for (int i = food_y * tile_size; i < food_y * tile_size + tile_size; 
    	i++) 
    {
        for (int j = food_x * tile_size; j < food_x * tile_size + border; 
        	j++) 
        {
            fb[LCD_width * i + j] = color;
        }
    ...
    for (int i = food_y * tile_size; i < food_y * tile_size + tile_size; 
    	i++) 
    {
        for (int j = food_x * tile_size + tile_size - border; 
        	j < food_x * tile_size + tile_size; j++) 
        {
            fb[LCD_width * i + j] = color;
        }
    }
    for (int i = food_y * tile_size + tile_size - border; 
    	i < food_y * tile_size + tile_size; i++) 
    {
        for (int j = food_x * tile_size; j < food_x * tile_size + tile_size; 
        	j++) 
        {
