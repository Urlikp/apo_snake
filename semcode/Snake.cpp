...
bool Snake::is_possible_die(int move_x, int move_y){
    ...
    if(opponent_snake!=NULL){
        for (size_t i = 0; i < opponent_snake->get_size()-1; i++)
        {
            Snake_Tile tmp_tile = opponent_snake->get_tile(i);
            Snake_Tile::Direction tmp_dir = tmp_tile.get_direction();
            printf("OP PREV%zu: x%d y%d\n", i, tmp_tile.get_x(), tmp_tile.get_y());
            Snake_Tile checked_tile = opponent_snake->get_tile(i+1);
            printf("OP CHECK %zu BEFORE increase: x%d y%d\n", i+1, checked_tile.get_x(), checked_tile.get_y());
            checked_tile.set_x(moves[tmp_dir].move_x + checked_tile.get_x());
            checked_tile.set_y(moves[tmp_dir].move_y + checked_tile.get_y());
            printf("OP CHECK %zu After increase: x%d y%d\n", i+1, checked_tile.get_x(), checked_tile.get_y());
            if (checked_tile.get_y()==my_head.get_y() || checked_tile.get_x()==my_head.get_x()){
                return true;
            }
        }
...
bool Snake::is_getting_closer_to_food(int move_x, int move_y){
...
Snake_Tile::Direction  Snake::choose_move(){
    ...
    for(it = moves.begin(); it!= moves.end(); it++){
        ...
        if(is_possible_eat_food(it->second.move_x, it->second.move_y)){
            it->second.reward += 0.05;
        }else{
            if(is_getting_closer_to_food(it->second.move_x, it->second.move_y)){
                printf("GETTING CLOSER\n");
                it->second.reward -=0.15;
            }else{
                printf("NO GETTING CLOSER\n");
                it->second.reward -=0.25;
            }
        }
...
void Snake::update(){
    if(size>0){
        printf("MOVE************************\n");
        printf("SIZE: %zu\n", size);
        Snake_Tile::Direction new_reward_dir = choose_move();
