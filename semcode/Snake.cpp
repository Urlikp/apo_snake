...
#include "Snake.h"
/*
All functions are described in Snake.h
*/
...
Snake_Tile Snake::get_tile(int pos)
{
    ...
    {
        fprintf(stderr, "Cannot get a tile of snake: position =%d but must be \
 				in range 0 to %zu.\n", pos, size);
        return Snake_Tile(-1, -1); //NULL
    }
}
...
bool Snake::is_new_direction_correct(Snake_Tile::Direction new_dir, 
...
bool Snake::is_tile_occupied(int x, int y)
{
	...
	{
		if (opponent_snake->get_tile(i).get_x() == x && 
			opponent_snake->get_tile(i).get_y() == y)
		{
			return true;
		}
	}
...
Snake_Tile::Direction Snake::choose_move2()
{
    ...
    for (int dirInt = Snake_Tile::UP; dirInt != Snake_Tile::NONE; dirInt++)
    {
        ...
        if (is_new_direction_correct(new_dir,dir))
        {
			...
			if (head_x >= 0 && head_x < gm.game_width / gm.size_of_tile && 
				head_y >= 0 && head_y < gm.game_height / gm.size_of_tile)
			{
				if (!is_tile_occupied(head_x, head_y))
				{
					int new_diff_x = food_x - head_x;
					int new_diff_y = food_y - head_y;
					float new_distance = (float)sqrt(pow(new_diff_x, 2) + 
													pow(new_diff_y, 2));
...
void Snake::fill_array(uint16_t* fb,  int LCD_width)
{
    for (size_t i = 0; i <= size; i++)
    {
        if (i < size)
        {
            Rectangle *rect = snake_tiles[i].get_occupied_field(tile_size, 
            													tile_size);
            for (int y = rect->y * rect->h; y < rect->y * rect->h + rect->h; y++)
            {
                for (int x  = rect->x * rect->w; x < rect->x * rect->w+rect->w; 
                	x++)
                {
    ...
    if(size > 0)
    {
        if(is_player_playing)
        {
            ...
            printf("MY HEAD: %s X%d Y%d S%zu **\n", 
            	snake_tiles[0].print_direction(), snake_tiles[0].get_x(), 
            	snake_tiles[0].get_y(), size);
        }
        else
        {
            ...
            snake_tiles[0].set_y(snake_tiles[0].get_y()+velocity_y);
            snake_tiles[0].set_x(snake_tiles[0].get_x()+velocity_x);
            printf("HEAD: %s X%d Y%d S%zu\n", snake_tiles[0].print_direction(), 
            	snake_tiles[0].get_x(), snake_tiles[0].get_y(), size);
        }
    }
    else
    {
